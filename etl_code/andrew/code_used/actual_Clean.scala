...
spark.sql("CREATE DATABASE IF NOT EXISTS aj3556_nyu_edu")

spark.sql("""
  CREATE EXTERNAL TABLE IF NOT EXISTS aj3556_nyu_edu.weather_cleaned (
    timestamp       timestamp,
    date            date,
    hour            int,
    month           int,
  ...
  STORED AS PARQUET
  LOCATION 'hdfs:///user/aj3556_nyu_edu/actual_hw6_dir'
""")

spark.sql("DESCRIBE aj3556_nyu_edu.weather_cleaned").show()
spark.sql("SELECT * FROM aj3556_nyu_edu.weather_cleaned LIMIT 10").show()